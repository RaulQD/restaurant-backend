...
export class UserModel {
    ...
    static async getUserById({ id }) {
        try {
            ...
            const [resultUser] = await pool.query(query, [id])
    ...
    static async getUserByEmail({ email }) {
        try {
            const [resultUser] = await pool.query('SELECT * FROM users WHERE email = ?', [email]);
            return resultUser;
        } catch (error) {
